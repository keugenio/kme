<template>
  <div
    class="d-flex flex-column align-items-center jusfity-content-center p-3 h-100"
  >
    ...
    <div class="modal-body">
      ...
      <div
        v-else
        class="mx-auto fa-5x"
        :class="[!hideOffScreen ? 'topZero' : 'topMinusNineThousand']"
      >
        <i class="fa fa-cog fa-spin text-primary"></i>
        <p style="font-size:2rem">
          downloading character information from Marvel Studios. Please be
          patient...
        </p>
      </div>
      <div
      ...
      >
        <div class="col-sm-6">
          ...
          <p id="instructions" v-if="gameInProgress">
          ...
          <hr />
          <button
            class="btn box-shadow btn-primary"
            ref="btnReload"
            @click="resetGame"
          >
            Reload
        ...
        <div class="col-sm-6">
          <figure class="d-table">
            <img
              alt="character image"
              class="rounded align-self-center w-100 h-auto"
              ref="marvelCharacterImage"
            />
            <figcaption
              class="caption align-bottom text-center d-table-caption"
              ref="marvelCopyright"
            ></figcaption>
          </figure>
        </div>
...
export default {
  ...
  methods: {
    ...
    newGame() {
      ...
      axios.get(htmlCall).then(function(downloadedJSON) {
        // filter out the characters that don't have a picture
        const results = downloadedJSON.data.data.results.filter(
          marvel =>
            marvel.thumbnail.path !=
              "http://i.annihil.us/u/prod/marvel/i/mg/2/70/5239be7020a11" &&
            marvel.thumbnail.path !=
              "http://i.annihil.us/u/prod/marvel/i/mg/b/40/image_not_available"
        );
        self.randomIndex = Math.floor(Math.random() * results.length + 1);
        var characterImage =
          results[self.randomIndex].thumbnail.path + "/portrait_uncanny.jpg";
        ...
        self.description =
          results[self.randomIndex].description ||
    ...
    evaluateGuess(guess) {
      ...
      if (self.randomCharacterNameLength == 0) {
        Swal.fire({
          ...
          text:
            "The character info was not downloaded correctly, please try again.",
          type: "error",
        ...
        if (indexOfGuess >= 0) {
          ...
          if (this.randomCharacterNameLength == 0) {
            ...
            if (this.description != "no description provided by Marvel API") {
              ...
              self.$refs.description.innerHTML +=
                "<a href='" +
                this.wikiURL +
                "' target='blank'>" +
                " (more info)" +
    ...
    updateMessageBoard() {
      var message = "";
      message +=
        "<p> Incorrect guessed amount: " + this.incorrectGuessAmt + "<br>";
      message +=
        "All guessed letters: " + this.allGuessArray.toString() + "<br></p>";
    ...
    keyMonitor: function(event) {
      ...
      if (
        this.isValidKey(event.key).length == 1 &&
        this.allGuessArray.indexOf(event.key) < 0
      ) {
